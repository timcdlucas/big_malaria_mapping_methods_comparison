...
mosq_comb <- fread('../../../data/other_response/combined_other_response.csv')

mosq_rows <- mosq_comb$response == 'mosq'

mosq <- fread('../../../data/other_response/mosquito_occurrence.csv')



#+ mosq_covs

covs_mosq <- fread('../../../data/extracted_covs/base_extra_response.csv')
covs_mosq <- 
  covs_mosq %>% 
    select(-V1) %>% 
    filter(mosq_rows)
